...
def test_capture_stdout(executor: SimpleLocalCodeExecutor,
                        code_str: str,
                        input_value: Any,
                        expected_stdout: str):
...
def test_no_main_gateway_exception(executor: SimpleLocalCodeExecutor, code_str: str):
    with pytest.raises(KeyError):
        executor.execute_code(code_str, "input_not_relevant")


@pytest.mark.parametrize("code_str", [main_not_callable])
def test_main_is_not_callable(executor: SimpleLocalCodeExecutor, code_str: str):
    with pytest.raises(TypeError):
        executor.execute_code(code_str, "input_not_relevant")


@pytest.mark.parametrize("code_str", [code_runtime_error])
def test_code_runtime_error(executor: SimpleLocalCodeExecutor, code_str: str):
    with pytest.raises(RuntimeError):
        executor.execute_code(code_str, "input_not_relevant")


@pytest.mark.parametrize("code_str", [code_syntax_error1, code_syntax_error2, code_syntax_error3])
def test_code_syntax_error(executor: SimpleLocalCodeExecutor, code_str: str):
